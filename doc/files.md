# African Bitcoin Core file system
...
The data directory is the default location where the African Bitcoin Core files are stored.
...
macOS    | `$HOME/Library/Application Support/African Bitcoin/`
Windows  | `%APPDATA%\African Bitcoin\` <sup>[\[1\]](#note1)</sup>

2. The non-default data directory path can be specified by `-datadir` option.

3. All content of the data directory, except for `africanbitcoin.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `africanbitcoin.conf`        | Contains [configuration settings](bitcoin-conf.md) for `africanbitcoind` or `africanbitcoin-qt`; can be specified by `-conf` option
`./`               | `africanbitcoind.pid`        | Stores the process ID (PID) of `africanbitcoind` or `africanbitcoin-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `africanbitcoind` or `africanbitcoin-qt`; can be specified by `-debuglogfile` option
...
`africanbitcoin-qt` uses [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).
