...
/** Timeout for (unprotected) outbound peers to sync to our chaintrust, in seconds */
...
static void FindNextBlocksToDownload(NodeId nodeid, unsigned int count, std::vector<const CBlockIndex*>& vBlocks, NodeId& nodeStaller, const Consensus::Params& consensusParams) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (state->pindexBestKnownBlock == nullptr || state->pindexBestKnownBlock->nChainTrust <= ::ChainActive().Tip()->nChainTrust || state->pindexBestKnownBlock->nChainTrust < nMinimumChainTrust) {
 ...
 * to compatible peers.
 */
void PeerManager::NewValidBlock(const CBlockIndex *pindex, const std::shared_ptr<const CBlock>& pblock) {
...
void PeerManager::ProcessHeadersMessage(CNode& pfrom, const std::vector<CBlockHeader>& headers, bool via_compact_block)
{
...

    {
        ...
        if (::ChainstateActive().IsInitialBlockDownload() && nCount != MAX_HEADERS_RESULTS) {
            ...
            if (nodestate->pindexBestKnownBlock && nodestate->pindexBestKnownBlock->nChainTrust < nMinimumChainTrust) {
                // This peer has too little work on their headers chain to help
                // us sync -- disconnect if it is an outbound disconnection
                // candidate.
                // Note: We compare their tip to nMinimumChainTrust (rather than
                // ::ChainActive().Tip()) because we won't start block download
                // until we have a headers chain that has at least
                // nMinimumChainTrust, even if a peer has a chain past our tip,
...
static bool IsValidNodeProtocolVersion(CNode& pfrom, const int nVersion, const CChainParams& chain_params)
{
    ...
    if (::ChainActive().Height() < chain_params.GetConsensus().AFBTCColdStakeEnableHeight) {
                                         ...
                                         const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::CMPCTBLOCK)
    {
        ...
        if (fBlockReconstructed) {
            ...
            // we have a chain with at least nMinimumChainTrust), and we ignore
    ...
    if (msg_type == NetMsgType::BLOCKTXN)
    {
        ...
        bool fBlockRead = false;
        {
            ...
            if (status == READ_STATUS_INVALID) {
                ...
                // 1. bad-proof-of-authority (impossible here, because we've already
                //    accepted the header)
...

    {
        ...
        if (!state.fSyncStarted && !pto->fClient && !fImporting && !fReindex) {
            ...
            if ((nSyncStarted == 0 && fFetch) || pindexBestHeader->GetBlockTime() > GetAdjustedTime() - 24 * 60 * 60) {
                ...
                state.nHeadersSyncTimeout = count_microseconds(current_time) + HEADERS_DOWNLOAD_TIMEOUT_BASE + HEADERS_DOWNLOAD_TIMEOUT_PER_HEADER * (GetAdjustedTime() - pindexBestHeader->GetBlockTime())/(consensusParams.nTargetSpacing);
                nSyncStarted++;
        ...
        if (state.vBlocksInFlight.size() > 0) {
            ...
            if (count_microseconds(current_time) > state.nDownloadingSince + consensusParams.nTargetSpacing * (BLOCK_DOWNLOAD_TIMEOUT_BASE + BLOCK_DOWNLOAD_TIMEOUT_PER_PEER * nOtherPeersWithValidatedDownloads)) {
