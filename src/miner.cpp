...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fProofOfStake, int64_t* pTotalFees, int32_t nTime, bool fAddTxs)
{
    ...
    pblock->nBits = chainparams.GetConsensus().powLimit.GetCompact();
    pblock->nNonce = 0;
    pblocktemplate->vTxSigOpsCost[0] = WITNESS_SCALE_FACTOR * GetLegacySigOpCount(*pblock->vtx[0]);
    pblock->hashMerkleRoot = BlockMerkleRoot(*pblock);

    if (fProofOfStake)
        pblock->nBits = GetNextBitsRequired(pindexPrev, pblock, chainparams.GetConsensus());
...
{
    // PoA signing key
    CKey signingKey = DecodeSecret(gArgs.GetArg("-publisherkey", ""));
    if (!signingKey.IsValid()) {
        throw std::runtime_error("Publisher secret is not configured");
    }

    // Append a signature to our block
    if (!signingKey.SignCompact(block.GetHashWithoutSign(), block.vchBlockSig) && CheckProofOfAuthority(block, Params().GetConsensus()))
...
std::shared_ptr<CBlock> SignAuhority(const CTxMemPool& mempool)
{
    ...
    // Used in coinbase outputs
    CScript scriptPubKey = GetScriptForDestination(WitnessV0KeyHash(signingKey.GetPubKey().GetID()));
    ...
    std::unique_ptr<CBlockTemplate> pblocktemplate(
        BlockAssembler(mempool, Params()).CreateNewBlock(scriptPubKey, false, &nTotalFees, GetAdjustedTime()));
    ...
    // Don't disable PoA publishing for no connections if in regtest mode
    if (!regtestMode && !gArgs.GetBoolArg("-emergencypublishing", false)) {
        // Don't process empty blocks unless current tip is old enough
        bool fOldEnough = ::ChainActive().Tip()->GetBlockTime() > GetTime() - Params().GetConsensus().nMinipumPoASpacing;
        if (fOldEnough && pblock->vtx.size() == 1) {
            throw std::runtime_error("Chain tip is not old enough to generate empty block");
        }
    }

    // Append a signature to our block
    bool success = signingKey.SignCompact(pblock->GetHashWithoutSign(), pblock->vchBlockSig) && CheckProofOfAuthority(*pblock, Params().GetConsensus());

    if (success) {
        //check timestamps
        if (pblock->GetBlockTime() <= pindexPrev->GetBlockTime() ||
            FutureDrift(pblock->GetBlockTime()) < pindexPrev->GetBlockTime()) {
            throw std::runtime_error("Valid PoA block took too long to create and has expired");
        }

        if (::ChainActive().Tip()->GetBlockHash() != pblock->hashPrevBlock) {
            //another block was received while building ours, scrap progress
            throw std::runtime_error("PoA block was orphaned before becoming valid");
        }
...
void ThreadStakeMiner(CWallet *pwallet, CConnman* connman, ChainstateManager* chainman, CTxMemPool* mempool)
{
    ...
    std::string threadName = "afbtcstake";
...
void StakeAFBTCs(bool fStake, CWallet *pwallet, CConnman* connman, ChainstateManager* chainman, CTxMemPool* mempool, boost::thread_group*& stakeThread)
{
