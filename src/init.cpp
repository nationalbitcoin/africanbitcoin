 ...
 */
static const char* BITCOIN_PID_FILENAME = "africanbitcoin.pid";
...
void SetupServerArgs(NodeContext& node)
{
    ...
    argsman.AddArg("-minimumchaintrust=<hex>", strprintf("Minimum work assumed to exist on a valid chain in hex (default: %s, testnet: %s)", defaultChainParams->GetConsensus().nMinimumChainTrust.GetHex(), testnetChainParams->GetConsensus().nMinimumChainTrust.GetHex()), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-port=<port>", strprintf("Listen for connections on <port> (default: %u, testnet: %, regtest: %u)", defaultChainParams->GetDefaultPort(), testnetChainParams->GetDefaultPort(), regtestChainParams->GetDefaultPort()), ArgsManager::ALLOW_ANY | ArgsManager::NETWORK_ONLY, OptionsCategory::CONNECTION);
    ...
    argsman.AddArg("-publisherkey=<wif>", "Set publisher private key for proof-of-authority", ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
    argsman.AddArg("-emergencypublishing", "Allow proof-of-authority publishing without timestamp or mempool checkings (default: 0)", ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/nationalbitcoin/africanbitcoin>";
...
bool AppInitParameterInteraction(const ArgsManager& args)
{
    ...
    if (args.IsArgSet("-minimumchaintrust")) {
        const std::string minChainTrustStr = args.GetArg("-minimumchaintrust", "");
        if (!IsHexNumber(minChainTrustStr)) {
            return InitError(strprintf(Untranslated("Invalid non-hex (%s) minimum chain trust value specified"), minChainTrustStr));
        }
        nMinimumChainTrust = UintToArith256(uint256S(minChainTrustStr));
    } else {
        nMinimumChainTrust = UintToArith256(chainparams.GetConsensus().nMinimumChainTrust);
    }
    LogPrintf("Setting nMinimumChainTrust=%s\n", nMinimumChainTrust.GetHex());
    if (nMinimumChainTrust < UintToArith256(chainparams.GetConsensus().nMinimumChainTrust)) {
        LogPrintf("Warning: nMinimumChainTrust set below default value of %s\n", chainparams.GetConsensus().nMinimumChainTrust.GetHex());
    }
    ...
    if (args.IsArgSet("-maxtipage")) {
        ...
        nMaxTipAge = std::min((int64_t)(0.8 * chainparams.GetConsensus().nTargetSpacing * COINBASE_MATURITY), DEFAULT_MAX_TIP_AGE);
    }
