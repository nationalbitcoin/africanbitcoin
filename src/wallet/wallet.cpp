...
bool CWallet::CreateCoinStake(const CWallet& wallet, unsigned int nBits, const CAmount& nTotalFees, uint32_t nTimeBlock, CMutableTransaction& tx, CKey& key, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoins)
{
    ...
    else if(!CreateMPoSOutputs(txNew, nRewardPiece, pindexPrev->nHeight, consensusParams))
    {
...
int CWalletTx::GetBlocksToMaturity() const
{
    ...
    //AFBTCTODO Some wallets are still not abandoning coinstakes correctly
...
void CWallet::StakeAFBTCs(bool fStake, CConnman* connman, ChainstateManager* chainman, CTxMemPool* mempool)
{
    ::StakeAFBTCs(fStake, this, connman, chainman, mempool, stakeThread);
}

void CWallet::StartStake(CConnman* connman, ChainstateManager* chainman, CTxMemPool* mempool)
{
    m_enabled_staking = true;
    StakeAFBTCs(true, connman, chainman, mempool);
}
